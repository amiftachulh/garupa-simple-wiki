export default [
  {
    id: 6,
    name: 'Kasumi Toyama',
    japanese_name: '戸山 香澄',
    image: 'https://i.bandori.party/u/i/6Kasumi-Toyama-Nhs7xX.png',
    square_image: 'https://i.bandori.party/u/i/m/6Kasumi-Toyama-jsVzTv.png',
    i_band: "Poppin'Party",
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'First',
    romaji_CV: 'Aimi',
    CV: '愛美',
    birthday: '2015-07-14',
    food_like: 'French fries, white rice',
    food_dislike: 'Natto (fermented soybeans)',
    i_astrological_sign: 'Cancer',
    instrument: 'Guitar & Vocals',
    description:
      "She is highly active and always positive girl. Since she entered high school, she's been searching for something sparkling that could make her heart flutter. Along the way, she encountered a girl with a bright red star-shaped guitar, and since then, it's become her dream to form a girl band. She has a younger sister in her family of 4, but it's not exactly clear who acts like the elder.",
  },
  {
    id: 7,
    name: 'Tae Hanazono',
    japanese_name: '花園 たえ',
    image: 'https://i.bandori.party/u/i/7Tae-Hanazono-lO6UJ6.png',
    square_image: 'https://i.bandori.party/u/i/m/7Tae-Hanazono-5ctAXt.png',
    i_band: "Poppin'Party",
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'First',
    romaji_CV: 'Otsuka Sae',
    CV: '大塚紗英',
    birthday: '2015-12-04',
    food_like: 'Hamburger steak, red-bean soup, basically anything!',
    food_dislike: 'None! (Nothing too weird, though...)',
    i_astrological_sign: 'Sagittarius',
    instrument: 'Guitar',
    description:
      'She started playing the guitar in elementary school, and is very skilled at it. As a huge lover of music, she started working part-time at a live house upon entering high school. Her favorite item is her blue guitar that she bought using money that she saved. A natural airhead who lives at her own pace, she sometimes surprises the people around her with her unexpected moves. She lives with her parents and 20 rabbits.',
  },
  {
    id: 8,
    name: 'Rimi Ushigome',
    japanese_name: '牛込 りみ',
    image: 'https://i.bandori.party/u/i/8Rimi-Ushigome-6h0QQU.png',
    square_image: 'https://i.bandori.party/u/i/m/8Rimi-Ushigome-gMEIOR.png',
    i_band: "Poppin'Party",
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'First',
    romaji_CV: 'Nishimoto Rimi',
    CV: '西本 りみ',
    birthday: '2015-03-23',
    food_like: 'Chocolate, meat, whipped cream',
    food_dislike: 'Mint-chocolate flavored food',
    i_astrological_sign: 'Aries',
    instrument: 'Bass',
    description:
      "During their high school entrance ceremony, she heard Kasumi's intro that was filled with boundless optimism, and this sparked her interest in Kasumi. Even though she wants to change her cowardly and shy personality, she has been unable to do anything about it. The pink bass that she uses is a hand-me-down from her elder sister Yuri, who is 2 years older than her and studies at the same high school. She is a huge fan of Yamabuki Bakery, and especially loves their chocolate cornet.",
  },
  {
    id: 9,
    name: 'Saaya Yamabuki',
    japanese_name: '山吹 沙綾',
    image: 'https://i.bandori.party/u/i/9Saaya-Yamabuki-mPTjhv.png',
    square_image: 'https://i.bandori.party/u/i/m/9Saaya-Yamabuki-7JDbNa.png',
    i_band: "Poppin'Party",
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'First',
    romaji_CV: 'Ohashi Ayaka',
    CV: '大橋 彩香',
    birthday: '2015-05-19',
    food_like: 'Spaghetti with Garlic and Chili Pepper, cheese',
    food_dislike: 'Raw seafood',
    i_astrological_sign: 'Taurus',
    instrument: 'Drums',
    description:
      "She became good friends with her classmate Kasumi during the high school entrance ceremony, and since then they often eat their bentos together. A kind person who cares deeply about her friends, she often lends an ear to Kasumi. She is also a filial girl, helping out at her family's bakery, Yamabuki Bakery, even during the school term. She has a younger brother and sister. She apparently used to play in a girls' band when she was still in middle school, but now...",
  },
  {
    id: 10,
    name: 'Arisa Ichigaya',
    japanese_name: '市ヶ谷 有咲',
    image: 'https://i.bandori.party/u/i/10Arisa-Ichigaya-i3p1ze.png',
    square_image: 'https://i.bandori.party/u/i/m/10Arisa-Ichigaya-OhdoPc.png',
    i_band: "Poppin'Party",
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'First',
    romaji_CV: 'Itou Ayasa',
    CV: '伊藤 彩沙',
    birthday: '2015-10-27',
    food_like: 'Anmitsu, brown rice, hard-boiled eggs',
    food_dislike: 'Anything with leeks',
    i_astrological_sign: 'Scorpio',
    instrument: 'Keyboard',
    description:
      'Her family runs a pawnshop called "Ryuuseidou". Being in indoor person, her hobbies include tending for bonsai plants and internet surfing. Even though she\'s basically a shut-in, she still manages to achieve excellent results in school. Because her sharp tongue, she often gets into arguments with others, especially Kasumi, but it\'s usually because she is unable to be honest with herself. She started learning the piano when she was young, but gave up on it halfway.',
  },
  {
    id: 11,
    name: 'Ran Mitake',
    japanese_name: '美竹 蘭',
    image: 'https://i.bandori.party/u/i/11Ran-Mitake-YXc3tt.png',
    square_image: 'https://i.bandori.party/u/i/m/11Ran-Mitake-N5oapw.png',
    i_band: 'Afterglow',
    school: "Haneoka Girls' High School",
    i_school_year: 'First',
    romaji_CV: 'Sakura Ayane',
    CV: '佐倉 綾音',
    birthday: '2015-04-10',
    food_like: 'Dark chocolate sweets',
    food_dislike: 'Peas',
    i_astrological_sign: 'Aries',
    instrument: 'Guitar & Vocals',
    description:
      "Ran is only child of the masters of a flower arrangement school that goes back over 100 years. She's strong-willed and hates to lose, but she also has a side of her that hates being alone. She's the type of person to treasure her childhood friends and family more than anyone else. She has no particular interests.",
  },
  {
    id: 12,
    name: 'Moca Aoba',
    japanese_name: '青葉 モカ',
    image: 'https://i.bandori.party/u/i/12Moca-Aoba-czB7B8.png',
    square_image: 'https://i.bandori.party/u/i/m/12Moca-Aoba-FSqhqR.png',
    i_band: 'Afterglow',
    school: "Haneoka Girls' High School",
    i_school_year: 'First',
    romaji_CV: 'Misawa Sachika',
    CV: '三澤 紗千香',
    birthday: '2015-09-03',
    food_like: 'Buns',
    food_dislike: 'Spicy food',
    i_astrological_sign: 'Virgo',
    instrument: 'Guitar',
    description:
      "Moca doesn't really have anything that she's super into, and is apathetic towards things that don't interest her, but she's the type who will get really worked up about the people she finds important. She likes to keep doing one thing without quitting, and playing her instrument is one of those things. She enjoys collecting point cards and sleeping.",
  },
  {
    id: 13,
    name: 'Himari Uehara',
    japanese_name: '上原 ひまり',
    image: 'https://i.bandori.party/u/i/13Himari-Uehara-LVtvvz.png',
    square_image: 'https://i.bandori.party/u/i/m/13Himari-Uehara-w2nYPj.png',
    i_band: 'Afterglow',
    school: "Haneoka Girls' High School",
    i_school_year: 'First',
    romaji_CV: 'Katou Emiri',
    CV: '加藤 英美里',
    birthday: '2015-10-23',
    food_like: 'Chocolate, convenience store sweets',
    food_dislike: 'Shiitake mushrooms',
    i_astrological_sign: 'Libra',
    instrument: 'Bass',
    description:
      "Himari is cheerful, good-natured, and is the band coordinator. She struggles to read the mood of a situation, which means she sometimes does things that have no effect. She's weak to tear-jerkers and sentimental episodes, and is easily moved to tears. She enjoys comparing the different sweets she buys from convenience stores.",
  },
  {
    id: 14,
    name: 'Tomoe Udagawa',
    japanese_name: '宇田川 巴',
    image: 'https://i.bandori.party/u/i/14Tomoe-Udagawa-WUlJ8p.png',
    square_image: 'https://i.bandori.party/u/i/m/14Tomoe-Udagawa-S7I3Tb.png',
    i_band: 'Afterglow',
    school: "Haneoka Girls' High School",
    i_school_year: 'First',
    romaji_CV: 'Hikasa Yoko',
    CV: '日笠 陽子',
    birthday: '2015-04-15',
    food_like: 'Pork and soy sauce ramen',
    food_dislike: 'None',
    i_astrological_sign: 'Aries',
    instrument: 'Drums',
    description:
      "Tomoe is a pretty chill person who never says anything bad about other people or holds grudges against them. She also keeps the band from falling apart. She's friends with the adults who work in the shopping district, and often attends her hometown's festivals to play the taiko drum. She is interested in fashion and taiko drumming. Her younger sister is Udagawa Ako.",
  },
  {
    id: 15,
    name: 'Tsugumi Hazawa',
    japanese_name: '羽沢 つぐみ',
    image: 'https://i.bandori.party/u/i/15Tsugumi-Hazawa-XRWlYl.png',
    square_image: 'https://i.bandori.party/u/i/m/15Tsugumi-Hazawa-KNbiyZ.png',
    i_band: 'Afterglow',
    school: "Haneoka Girls' High School",
    i_school_year: 'First',
    romaji_CV: 'Kanemoto Hisako',
    CV: '金元 寿子',
    birthday: '2015-01-07',
    food_like: "Her mother's home baked cakes",
    food_dislike: 'Black coffee',
    i_astrological_sign: 'Capricorn',
    instrument: 'Keyboard',
    description:
      "Tsugumi is the most normal girl in the band. However, it's because she's so \"normal\" that she's such a hardworking and optimistic person who rarely gets discouraged. Her optimism really buoys the other members' spirits. She likes collecting bath powder.",
  },
  {
    id: 16,
    name: 'Kokoro Tsurumaki',
    japanese_name: '弦巻 こころ',
    image: 'https://i.bandori.party/u/i/16Kokoro-Tsurumaki-cY9bh4.png',
    square_image: 'https://i.bandori.party/u/i/m/16Kokoro-Tsurumaki-pS7FIm.png',
    i_band: 'Hello, Happy World!',
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'First',
    romaji_CV: 'Miku Itou',
    CV: '美来 伊藤',
    birthday: '2015-08-08',
    food_like: 'Anything at all!',
    food_dislike: 'None!',
    i_astrological_sign: 'Leo',
    instrument: 'Vocals',
    description:
      'Kokoro loves watching people smile, and her goal in life is to make the world smile. Despite her appearance, she is the only daughter of an extremely wealthy family. Her hobby is looking for fun things to do. Whenever she finds something that makes her feel "I want to do this!", she will do whatever it takes to make it come true. She\'s very curious, and her eyes are always dazzling.',
  },
  {
    id: 17,
    name: 'Kaoru Seta',
    japanese_name: '瀬田 薫',
    image: 'https://i.bandori.party/u/i/17Kaoru-Seta-iUNJsL.png',
    square_image: 'https://i.bandori.party/u/i/m/17Kaoru-Seta-RHusGK.png',
    i_band: 'Hello, Happy World!',
    school: "Haneoka Girls' High School",
    i_school_year: 'Second',
    romaji_CV: 'Tadokoro Azusa',
    CV: '田所 あずさ',
    birthday: '2015-02-28',
    food_like:
      '[In character] Grape juice (in wine glass), Sachertorte, Vichyssoise [For real] Rice cakes miso soup',
    food_dislike: 'Raw seafood',
    i_astrological_sign: 'Pisces',
    instrument: 'Guitar',
    description:
      'Kaoru is a celebrity at her school and belongs to the theater department there. She is very popular with ladies, who she dubs her "little kittens". In particular, Uehara Himari and Rimi Ushigome have a tendency of blushing furiously in her presence. She likes to read philosophical books and poetry, but doesn\'t really understand it. Outside of the band, she is also friends with Shirasagi Chisato.',
  },
  {
    id: 18,
    name: 'Hagumi Kitazawa',
    japanese_name: '北沢 はぐみ',
    image: 'https://i.bandori.party/u/i/18Hagumi-Kitazawa-Z16JI6.png',
    square_image: 'https://i.bandori.party/u/i/m/18Hagumi-Kitazawa-A21E3r.png',
    i_band: 'Hello, Happy World!',
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'First',
    romaji_CV: 'Yoshida Yuri',
    CV: '吉田 有里 ',
    birthday: '2015-07-30',
    food_like: 'Kitazawa croquettes, meat, curry',
    food_dislike: 'None',
    i_astrological_sign: 'Leo',
    instrument: 'Bass',
    description:
      "Hagumi is always bright, active, but sensitive with a pure heart. She sometimes helps out at her parents' butcher shop, which is famous for their croquettes. Her favourite activity is running marathons and she's the captain and ace of the local softball team. She's in the same class as Toyama Kasumi.",
  },
  {
    id: 19,
    name: 'Kanon Matsubara',
    japanese_name: '松原 花音',
    image: 'https://i.bandori.party/u/i/19Kanon-Matsubara-lNAkjb.png',
    square_image: 'https://i.bandori.party/u/i/m/19Kanon-Matsubara-gUFIXE.png',
    i_band: 'Hello, Happy World!',
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'Second',
    romaji_CV: 'Toyota Moe',
    CV: '豊田 萌絵',
    birthday: '2015-05-11',
    food_like: 'Delicious snacks, cake, coffee, tea',
    food_dislike: 'Mushrooms',
    i_astrological_sign: 'Taurus',
    instrument: 'Drums',
    description:
      "Kanon is very clumsy, quick to tears, and has a bad sense of direction. She's a bit of a pushover and very shy. Since she's a pushover, it's easy for her to get involved in others' affairs. She likes sweets and cute things, and is a bit reserved. She enjoys seeing jellyfish and going to cafes.",
  },
  {
    id: 20,
    name: 'Misaki Okusawa',
    japanese_name: '奥沢 美咲',
    image: 'https://i.bandori.party/u/i/20Misaki-Okusawa-MHj2mC.png',
    square_image: 'https://i.bandori.party/u/i/m/20Michelle-dbNp0x.png',
    i_band: 'Hello, Happy World!',
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'First',
    romaji_CV: 'Kurosawa Tomoyo',
    CV: '黒沢 ともよ',
    birthday: '2015-10-01',
    food_like: 'Family-style dish',
    food_dislike: 'Coriander',
    i_astrological_sign: 'Libra',
    instrument: 'DJ',
    description:
      'Known to her band mates as the pink bear Michelle, Misaki is a reluctant member of Hello, Happy World! Misaki began wearing the bear costume at her part-time job and she was roped into joining the band by Tsurumaki Kokoro.',
  },
  {
    id: 21,
    name: 'Aya Maruyama',
    japanese_name: '丸山 彩',
    image: 'https://i.bandori.party/u/i/21Aya-Maruyama-CZBkjr.png',
    square_image: 'https://i.bandori.party/u/i/m/21Aya-Maruyama-Zm0u9A.png',
    i_band: 'Pastel*Palettes',
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'Second',
    romaji_CV: 'Maeshima Ami',
    CV: '前島亜美',
    birthday: '2015-12-27',
    food_like: 'Hamburger steak, rice omelets',
    food_dislike: 'Octopus',
    i_astrological_sign: 'Capricorn',
    instrument: 'Vocals',
    description:
      "Aya admires idols and entered the entertainment industry as a probationary idol in the hopes of becoming one; however, she is easily nervous and cries easily when things go wrong and she's overwhelmed by her emotions. She has stage fright and so she has weak improvisation, so she has yet to achieve a breakthrough. After three years, she finally gets her chance to become an idol by becoming a part of Pastel*Palettes.",
  },
  {
    id: 22,
    name: 'Hina Hikawa',
    japanese_name: '氷川 日菜',
    image: 'https://i.bandori.party/u/i/22Hina-Hikawa-HEciRu.png',
    square_image: 'https://i.bandori.party/u/i/m/22Hina-Hikawa-2trHlE.png',
    i_band: 'Pastel*Palettes',
    school: "Haneoka Girls' High School",
    i_school_year: 'Second',
    romaji_CV: 'Ozawa Ari',
    CV: '小澤亜李',
    birthday: '2015-03-20',
    food_like: 'Gum, candy, junk food',
    food_dislike: 'Tofu skins and other flavorless foods',
    i_astrological_sign: 'Pisces',
    instrument: 'Guitar',
    description:
      "A bright and quick-witted genius of a girl, she's capable of doing something after she's seen it once. She has a happy and refreshing personality, but she can also be insensitive and isn't good at sensing the mood. She has a habit of describing things with unusual ideophones. She has a hobby of making perfumes. She's the younger twin sister of Hikawa Sayo.",
  },
  {
    id: 23,
    name: 'Chisato Shirasagi',
    japanese_name: '白鷺 千聖',
    image: 'https://i.bandori.party/u/i/23Chisato-Shirasagi-48NtZE.png',
    square_image:
      'https://i.bandori.party/u/i/m/23Chisato-Shirasagi-NtBBCb.png',
    i_band: 'Pastel*Palettes',
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'Second',
    romaji_CV: 'Uesaka Sumire',
    CV: '上坂 すみれ',
    birthday: '2015-04-06',
    food_like: 'Tea, refreshing flavors, acai fruit',
    food_dislike: 'Natto (fermented soybeans)',
    i_astrological_sign: 'Aries',
    instrument: 'Bass',
    description:
      'A young actress who has been active in the field since her roles as a child actress. She is polite and very responsible. Prior to joining Pastel Palettes, she was a child actress and did not know how to play the bass. She cherishes friends whom she can hang out with without needing to hold back. Her hobbies include shopping and having tea at cafes. Outside of the band, she is also friends with Matsubara Kanon and Seta Kaoru.',
  },
  {
    id: 24,
    name: 'Maya Yamato',
    japanese_name: '大和 麻弥',
    image: 'https://i.bandori.party/u/i/24Maya-Yamato-x5G0HS.png',
    square_image: 'https://i.bandori.party/u/i/m/24Maya-Yamato-kNYTzU.png',
    i_band: 'Pastel*Palettes',
    school: "Haneoka Girls' High School",
    i_school_year: 'Second',
    romaji_CV: 'Nakagami Ikumi',
    CV: '育実 中上',
    birthday: '2015-11-03',
    food_like: 'Veggie sticks',
    food_dislike: 'Haute cuisine',
    i_astrological_sign: 'Scorpio',
    instrument: 'Drums',
    description:
      "An equipment geek, Maya is happiest when she's surrounded by instrument equipment. When you talk about instrument equipment, she becomes talkative. Prior to joining the band, she was a studio musician. She wears glasses, which she removes when performing.",
  },
  {
    id: 25,
    name: 'Eve Wakamiya',
    japanese_name: '若宮 イヴ',
    image: 'https://i.bandori.party/u/i/25Eve-Wakamiya-oUfPJf.png',
    square_image: 'https://i.bandori.party/u/i/m/25Eve-Wakamiya-jfMYo6.png',
    i_band: 'Pastel*Palettes',
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'First',
    romaji_CV: 'Hata Sawako',
    CV: '秦 佐和子',
    birthday: '2015-06-27',
    food_like: 'Gingerbread cookies',
    food_dislike: 'Pickled foods',
    i_astrological_sign: 'Cancer',
    instrument: 'Keytar',
    description:
      "Eve has an honest and friendly personality to everyone. She is in the same class as Kasumi Toyama and many other members of Poppin' Party. Before joining Pastel*Palettes, she was a model at the same agency. She is notably taller than the other members of her band. Eve is from Finland and finds Japanese culture exciting, often citing bushido, the code of Japanese warriors. She strongly admires Bushido. Her father is Japanese, and her mother, Finnish.",
  },
  {
    id: 26,
    name: 'Yukina Minato',
    japanese_name: '湊 友希那',
    image: 'https://i.bandori.party/u/i/26Yukina-Minato-dnhC8e.png',
    square_image: 'https://i.bandori.party/u/i/m/26Yukina-Minato-hUboXl.png',
    i_band: 'Roselia',
    school: "Haneoka Girls' High School",
    i_school_year: 'Second',
    romaji_CV: 'Aiba Aina',
    CV: '相羽 あいな',
    birthday: '2015-10-26',
    food_like: 'Honey tea, Cookies made by Lisa',
    food_dislike: 'Bitter melon (and other bitter things)',
    i_astrological_sign: 'Scorpio',
    instrument: 'Vocals',
    description:
      "Yukina is a powerful vocalist, sparking the admiration of the other Roselia members. She has a strong-willed and stubborn personality and doesn't smile much in public. She pursues her beliefs with reckless devotion, causing her to be blind to everything else around her. She is best friends with Imai Lisa and considers Mitake Ran somewhat of a rival. She has a weak spot for cats.",
  },
  {
    id: 27,
    name: 'Sayo Hikawa',
    japanese_name: '氷川 紗夜',
    image: 'https://i.bandori.party/u/i/27Sayo-Hikawa-2Jyxse.png',
    square_image: 'https://i.bandori.party/u/i/m/27Sayo-Hikawa-2g2xDG.png',
    i_band: 'Roselia',
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'Second',
    romaji_CV: 'Kudō Haruka',
    CV: '工藤晴香',
    birthday: '2015-03-20',
    food_like: 'Junk food, gum, candy',
    food_dislike: 'Carrots',
    i_astrological_sign: 'Pisces',
    instrument: 'Guitar',
    description:
      "A serious and fussy girl, Sayo can come off as somewhat cold in nature. She would never cut corners or be shoddy about things; because of that, she lives her life too seriously for her own good. She likes fluffy animals and practices archery. She's the older twin sister of Hikawa Hina. She has a complex and feels inferior to her twin sister.",
  },
  {
    id: 28,
    name: 'Lisa Imai',
    japanese_name: '今井 リサ',
    image: 'https://i.bandori.party/u/i/28Lisa-Imai-h8m3h3.png',
    square_image: 'https://i.bandori.party/u/i/m/28Lisa-Imai-NSapWM.png',
    i_band: 'Roselia',
    school: "Haneoka Girls' High School",
    i_school_year: 'Second',
    romaji_CV: 'Nakashima Yuki',
    CV: '中島由貴',
    birthday: '2015-08-25',
    food_like: 'Chicken stew, sour foods',
    food_dislike: 'Green smoothies',
    i_astrological_sign: 'Virgo',
    instrument: 'Bass',
    description:
      'With her flamboyant looks, Lisa is easily misunderstood as frivolous, but is actually a compassionate and faithful person. She has a lot of friends due to her stylishness and bright personality. Lisa follows gyaru fashion, but keeps her interest in knitting a secret, since it does not match her image. She is a proficient bassist, wowing the other Roselia members in her audition, and is good at baking cookies. She is best friends with Minato Yukina. Outside of the band, she is also friends with Aoba Moca.\n\nPreviously voiced by Endō Yurika (遠藤 ゆりか) until 13 May, where she stepped down due to health concerns and left the industry entirely.',
  },
  {
    id: 29,
    name: 'Ako Udagawa',
    japanese_name: '宇田川 あこ',
    image: 'https://i.bandori.party/u/i/29Ako-Udagawa-DJZTCz.png',
    square_image: 'https://i.bandori.party/u/i/m/29Ako-Udagawa-resmOR.png',
    i_band: 'Roselia',
    school: "Haneoka Girls' Junior High School",
    i_school_year: 'JrThird',
    romaji_CV: ' Sakuragawa Megu',
    CV: '桜川 めぐ',
    birthday: '2015-07-03',
    food_like: 'Potato chips, jelly beans',
    food_dislike: 'Eggs, peppers',
    i_astrological_sign: 'Cancer',
    instrument: 'Drums',
    description:
      'Ako has chuunibyou tendencies, which Shirokane Rinko supposes results from her playing online games. She often wonders how she can be a "cooler" drummer, and sometimes feels inadequate compared to her other band mates. She admires Minato Yukina, is the best friend of Shirokane Rinko, and the little sister of Udagawa Tomoe. She looks up to her sister, and started playing drums because she wanted to be as cool as her sister. She always is researching ways to be cooler, and started using weird words as a side-effect.',
  },
  {
    id: 30,
    name: 'Rinko Shirokane',
    japanese_name: '白金 燐子',
    image: 'https://i.bandori.party/u/i/30Rinko-Shirokane-ai2NDQ.png',
    square_image: 'https://i.bandori.party/u/i/m/30Rinko-Shirokane-S5HF59.png',
    i_band: 'Roselia',
    school: "Hanasakigawa Girls' Academy",
    i_school_year: 'Second',
    romaji_CV: 'Shizaki Kanon',
    CV: '志崎樺音',
    birthday: '2015-10-17',
    food_like: 'Warm milk',
    food_dislike: 'Celery',
    i_astrological_sign: 'Libra',
    instrument: 'Keyboard',
    description:
      'Rinko is very shy and quiet, and at first is scared of the idea of performing in a live concert. She has an easily withdrawn and pessimistic personality, but is also the type to go to extremes for things she has decided on. Her hobbies are reading and playing online games, the latter through which she met her best friend, Udagawa Ako. She also enjoys crossword puzzles. Previously voiced by Akesaka Satomi (明坂聡美) until she stepped down following Roselia Fan Meet 13 September due to sudden deafness.',
  },
  {
    id: 31,
    name: 'Mashiro Kurata',
    japanese_name: '倉田 ましろ',
    image: 'https://i.bandori.party/u/i/31Mashiro-Kurata-5yrKez.png',
    square_image: 'https://i.bandori.party/u/i/m/31Mashiro-Kurata-5theuT.png',
    i_band: 'Morfonica',
    school: "Tsukinomori Girls' High School",
    i_school_year: 'First',
    romaji_CV: 'Amane Shindo',
    CV: '進藤あまね',
    birthday: '2015-02-19',
    food_like: 'Beef Stew',
    food_dislike: null,
    i_astrological_sign: 'Pisces',
    instrument: 'Vocals',
    description:
      "A quiet and reserved girl. Has an active imagination and tends to get caught up in daydreams. She had high hopes to go to the prestigious Tsukinomori Girls' High School, but upon getting in, became overwhelmed by the gifted students surrounding her. She spent her days in sorrow until she met a certain band...\n",
  },
  {
    id: 32,
    name: 'Touko Kirigaya',
    japanese_name: '桐ヶ谷 透子',
    image: 'https://i.bandori.party/u/i/32Kirigaya-Touko-6RQKuZ.png',
    square_image: 'https://i.bandori.party/u/i/m/32Kirigaya-Touko-roXDxC.png',
    i_band: 'Morfonica',
    school: "Tsukinomori Girls' High School",
    i_school_year: 'First',
    romaji_CV: 'Hina Suguta',
    CV: '直田姫奈',
    birthday: '2015-12-16',
    food_like: null,
    food_dislike: null,
    i_astrological_sign: 'Sagittarius',
    instrument: 'Guitar',
    description:
      'A popular girl who draws crowds in both school and social media. Has a bright and sunny personality, but her random tendencies often lead to others getting caught in the crossfire.\n',
  },
  {
    id: 33,
    name: 'Nanami Hiromachi',
    japanese_name: '広町 七深',
    image: 'https://i.bandori.party/u/i/33Nanami-Hiromachi-b7eoVy.png',
    square_image: 'https://i.bandori.party/u/i/m/33Nanami-Hiromachi-6p9s2u.png',
    i_band: 'Morfonica',
    school: "Tsukinomori Girls' High School",
    i_school_year: 'First',
    romaji_CV: 'Yuka Nishio',
    CV: '西尾夕香',
    birthday: '2015-06-16',
    food_like: null,
    food_dislike: 'None',
    i_astrological_sign: 'Gemini',
    instrument: 'Bass',
    description:
      'An odd girl who wishes to be normal. Her self-consciousness leads her to hide her true self in order to blend in.\n',
  },
  {
    id: 34,
    name: 'Tsukushi Futaba',
    japanese_name: '二葉 つくし',
    image: 'https://i.bandori.party/u/i/34Tsukushi-Futaba-EAM8aA.png',
    square_image: 'https://i.bandori.party/u/i/m/34Tsukushi-Futaba-q5lGFE.png',
    i_band: 'Morfonica',
    school: "Tsukinomori Girls' High School",
    i_school_year: 'First',
    romaji_CV: 'mika',
    CV: 'mika',
    birthday: '2015-09-15',
    food_like: null,
    food_dislike: null,
    i_astrological_sign: 'Virgo',
    instrument: 'Drums',
    description:
      "The ever-confident class rep. Despite her best efforts, her inner clumsiness sometimes boils over. She's working hard to be more dependable.\n",
  },
  {
    id: 35,
    name: 'Rui Yashio',
    japanese_name: '八潮 瑠唯',
    image: 'https://i.bandori.party/u/i/35Rui-Yashio-aQiGXe.png',
    square_image: 'https://i.bandori.party/u/i/m/35Rui-Yashio-VY7tDV.png',
    i_band: 'Morfonica',
    school: "Tsukinomori Girls' High School",
    i_school_year: 'First',
    romaji_CV: 'Ayasa',
    CV: 'Ayasa',
    birthday: '2015-11-19',
    food_like: null,
    food_dislike: null,
    i_astrological_sign: 'Scorpio',
    instrument: 'Violin',
    description:
      "A talented student at the top of her class. With her eyes always on the prize, her personality ends up being a little dry. She thought music to be a fool's endeavor, but that changed when she met Mashiro and the others...\n",
  },
  {
    id: 36,
    name: 'LAYER',
    japanese_name: 'LAYER',
    image: 'https://i.bandori.party/u/i/36LAYER-7C7HgY.png',
    square_image: 'https://i.bandori.party/u/i/m/36LAYER-rxqBxn.png',
    i_band: 'RAISE A SUILEN',
    school: null,
    i_school_year: 'Second',
    romaji_CV: 'Raychell',
    CV: null,
    birthday: '2015-01-13',
    food_like: 'Hot milk (with honey)',
    food_dislike: 'Bitter coffee, sea cucumbers',
    i_astrological_sign: 'Capricorn',
    instrument: 'Vocals & Bass',
    description:
      'A professional diva who is capable of singing any song, Rei is in charge of Vocals and Bass. She exerts an adult demeanor while keeping her distance from most people. She is childhood friends with Tae Hanazono.',
  },
  {
    id: 37,
    name: 'LOCK',
    japanese_name: 'LOCK',
    image: 'https://i.bandori.party/u/i/37LOCK-gn9xbt.png',
    square_image: 'https://i.bandori.party/u/i/m/37LOCK-hbUncH.png',
    i_band: 'RAISE A SUILEN',
    school: "Haneoka Girls' High School",
    i_school_year: 'First',
    romaji_CV: 'Riko Kohara',
    CV: '小原 莉子',
    birthday: '2015-07-17',
    food_like: null,
    food_dislike: null,
    i_astrological_sign: 'Cancer',
    instrument: 'Guitar',
    description:
      'A hardworking girl who struggles, but keeps at it. She works part time at the livehouse, "Galaxy" in the shopping district. She idolizes Poppin\'Party and chased after them to Tokyo for her studies.',
  },
  {
    id: 38,
    name: 'MASKING',
    japanese_name: 'MASKING',
    image: 'https://i.bandori.party/u/i/38MASKING-8Dukva.png',
    square_image: 'https://i.bandori.party/u/i/m/38MASKING-p5C4rr.png',
    i_band: 'RAISE A SUILEN',
    school: "Shirayuki Girls' Academy",
    i_school_year: 'Second',
    romaji_CV: 'Natsume',
    CV: '夏芽',
    birthday: '2015-05-12',
    food_like: 'Ramen',
    food_dislike: null,
    i_astrological_sign: 'Taurus',
    instrument: 'Drums',
    description:
      'Masuki is the daughter of the owner of the livehouse, "Galaxy", and has a part time job making Ramen. Her skills are unrivaled with the drums, where her nickname is "Mad Dog". She becomes lost in improvisation, and gets scolded by CHU² for it quite regularly. Masuki previously worked with Rei.\n',
  },
  {
    id: 39,
    name: 'PAREO',
    japanese_name: 'PAREO',
    image: 'https://i.bandori.party/u/i/39PAREO-EF21OQ.png',
    square_image: 'https://i.bandori.party/u/i/m/39PAREO-9NivgR.png',
    i_band: 'RAISE A SUILEN',
    school: null,
    i_school_year: 'JrSecond',
    romaji_CV: 'Reo Kurachi',
    CV: '倉知 玲鳳',
    birthday: '2015-03-25',
    food_like: 'Cabbage',
    food_dislike: null,
    i_astrological_sign: 'Aries',
    instrument: 'Keyboard',
    description:
      'Ever grateful to CHU² for scouting her, Pareo insists she was saved from "the darkness" when they met. For these reasons, she calls CHU² by the suffix, "-sama". Very particular about "Cute" auras, and loves Pastel*Palettes.\r\n',
  },
  {
    id: 40,
    name: 'CHU²',
    japanese_name: 'CHU²',
    image: 'https://i.bandori.party/u/i/40CHU²-N46MbU.png',
    square_image: 'https://i.bandori.party/u/i/m/40CHU²-OuDbvK.png',
    i_band: 'RAISE A SUILEN',
    school: "Celosia Girls' Academy",
    i_school_year: 'First',
    romaji_CV: 'Risa Tsumugi',
    CV: '紡木 吏佐',
    birthday: '2015-12-07',
    food_like: 'Jerky',
    food_dislike: 'Salad',
    i_astrological_sign: 'Sagittarius',
    instrument: 'DJ',
    description:
      'A child prodidgy who skipped grades and attends an international school of high caliber. She enjoys making the world\'s best music with DTM programs. Her absolute goal is to "change the world" with her music. Despite being arrogant, she is very mindful of her manners.',
  },
];
